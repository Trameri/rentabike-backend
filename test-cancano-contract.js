// Test specifico per creare contratto Cancano
async function testCancanoContract() {
  try {
    console.log('🧪 === TEST CONTRATTO CANCANO ===\n');

    // 1. Login come cancano
    console.log('🔐 1. Login come cancano...');
    ...
    const closeResponse = await fetch(`http://localhost:4000/api/contracts/${newContract._id}/close`, {
      ...
      body: JSON.stringify({
        ...
        paymentMethod: 'cash',
        isPaid: true,
        closureNotes: 'Test chiusura Cancano'
      })
    });
    ...
    const cancanoStats = stats.locations.find(l => l.location.code === 'cancano');
    
    console.log(`🏢 Cancano - Revenue: €${cancanoStats.revenue} | Contratti: ${cancanoStats.closedContracts}`);
